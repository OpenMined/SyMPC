...
class ABY3(metaclass=Protocol):
    ...
    def A2B(x: MPCTensor) -> List[MPCTensor]:
        """Convert an Arithmetic Shared Value to a Binary Shared Value To
        preserve space each Binary Shared Value is kept on a bit."""
        ...
        shares_p0 = [
            decompose(args[0][0].get().tensor, 2 ** 64),
            decompose(args[0][1].get().tensor, 2 ** 64),
        ]
        shares_p1 = [
            decompose(args[1][0].get().tensor, 2 ** 64),
            decompose(args[1][1].get().tensor, 2 ** 64),
        ]
        # stdlib
    ...
    def sum_shares(*shares: List[ShareTensor]) -> List[ShareTensor]:
        """Sum the shares such that each party would have in the end a Binary
        Shared Value.
